  ...
  return (
    <article className="dashboard-card-motion group relative flex min-h-[354px] flex-col overflow-hidden rounded-[24px] border border-white/10 bg-[#0b1020]/95 p-5 shadow-[0_18px_46px_rgba(4,7,18,0.3)]">
      ...
      <div className="relative mt-auto">
        ...
        <Button
          onClick={onView}
          className="mt-4 h-10 w-full rounded-2xl bg-gradient-to-r from-violet-500 via-fuchsia-500 to-sky-500 text-sm font-bold text-white shadow-[0_14px_30px_rgba(124,58,237,0.28)] transition-all duration-300 hover:-translate-y-0.5 hover:from-violet-400 hover:via-fuchsia-400 hover:to-sky-400 hover:shadow-[0_18px_38px_rgba(14,165,233,0.26)]"
        >
...
function CounsellorProfile({ counsellor, loading, selectedPlanId, setSelectedPlanId, booking, onBack, onSchedule }) {
  ...
  const availability = counsellor?.availability?.length
    ? counsellor.availability.slice(0, 4)
    : ["Monday: 10:00-16:00", "Wednesday: 14:00-17:00", "Friday: 09:00-14:00"];
    ...
    <div className="min-h-screen bg-[#050914] text-foreground">
      ...
      <main className="pt-16">
        <section className="dashboard-motion relative overflow-hidden py-10 md:py-12">
          ...
          <div className="absolute right-8 top-44 h-64 w-64 rounded-full bg-sky-500/10 blur-3xl" />
          <div className="relative mx-auto max-w-[1180px] px-4 sm:px-6 lg:px-8">
            <button type="button" onClick={onBack} className="mb-7 flex items-center gap-2 text-sm text-slate-300 transition hover:text-white">
            ...
            {loading && !counsellor ? (
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_350px]">
                <div className="space-y-5">
                  <Card className="dashboard-card-motion overflow-hidden rounded-[30px] border-white/10 bg-[#0b1020] shadow-[0_24px_70px_rgba(4,7,18,0.34)]">
                    <CardContent className="relative p-0">
                      <div className="absolute inset-x-0 top-0 h-24 bg-gradient-to-r from-violet-500/25 via-blue-500/15 to-cyan-400/10" />
                      <div className="relative p-5 md:p-6">
                        <div className="flex flex-col gap-5 md:flex-row md:items-start">
                          <div className="relative shrink-0">
                            <Avatar className="h-24 w-24 rounded-[26px] border-4 border-[#0b1020] bg-gradient-to-br from-orange-200 to-amber-300 text-violet-500 shadow-2xl">
                              <AvatarImage src={counsellor.profilePhotoUrl} alt={counsellor.name} />
                              <AvatarFallback className="rounded-[22px] bg-gradient-to-br from-orange-200 to-amber-300 text-3xl font-bold text-violet-500">
                                {initials(counsellor.name)}
                              </AvatarFallback>
                            </Avatar>
                            <span className="absolute -bottom-1 -right-1 h-5 w-5 rounded-full border-[4px] border-[#0b1020] bg-emerald-400" />
                          </div>

                          <div className="min-w-0 flex-1 pt-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <Badge className="rounded-full bg-emerald-400/15 text-emerald-300">
                                <ShieldCheck className="mr-1 h-3.5 w-3.5" />
                                {counsellor.badge || "Verified Professional"}
                              </Badge>
                              <Badge className="rounded-full bg-blue-400/15 text-blue-200">Accepting bookings</Badge>
                            </div>
                            <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-[2.35rem]">{counsellor.name}</h1>
                            ...
                            <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-300">
                              <span className="flex items-center gap-2 font-semibold">
                                ...
                                {Number(counsellor.rating || 4.8).toFixed(1)}
                                <span className="text-slate-400">({counsellor.reviews || 0} reviews)</span>
                              </span>
                              ...
                              <span className="flex items-center gap-2">
                                ...
                                {counsellor.responseTime || "Within 24 hours"}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-6 flex flex-wrap gap-2">
                          {categories.slice(0, 6).map((item) => (
                            <span key={item} className="rounded-full bg-[#151b30] px-3 py-1.5 text-xs font-semibold text-slate-100">
                              {item}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="mt-6 grid gap-3 md:grid-cols-3">
                          ...
                          <InfoTile icon={GraduationCap} label="Qualification" value={counsellor.education || "Verified qualification"} />
                          <InfoTile icon={Languages} label="Languages" value={languages.join(", ")} />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <div className="grid gap-5 md:grid-cols-[1.08fr_0.92fr]">
                    <Card className="dashboard-card-motion rounded-[24px] border-white/10 bg-[#0b1020]">
                      ...
                      <CardContent className="px-5 pb-5">
                        ...
                        <div className="mt-5 grid gap-3 sm:grid-cols-2">
                          <MiniPoint icon={MessageCircle} title="Care style" text="Supportive, practical, privacy-first" />
                          <MiniPoint icon={Video} title="Session modes" text={consultationModes.map(modeLabel).join(", ")} />
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="dashboard-card-motion rounded-[24px] border-white/10 bg-[#0b1020]">
                      <CardHeader className="p-5 pb-2">
                        ...
                        <CardDescription>Typical working windows and quick slots.</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-2 px-5 pb-5">
                        {availability.map((item) => (
                          <div key={item} className="rounded-2xl border border-white/8 bg-[#070b15] px-3 py-2 text-sm text-slate-300">
                            {item}
                          </div>
                        ))}
                      </CardContent>
                  ...
                  <Card className="dashboard-card-motion rounded-[26px] border-white/10 bg-[#0b1020]">
                    <CardHeader className="p-5 pb-3 md:p-6 md:pb-3">
                      <CardTitle className="text-xl">Therapy plans</CardTitle>
                      <CardDescription>Choose one affordable support package, then schedule date and time separately.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 md:grid-cols-3 md:px-6 md:pb-6">
                      {planList.map((plan) => (
                        <button
                          ...
                          className={`relative min-h-[220px] overflow-hidden rounded-[22px] border p-4 text-left transition ${
                        ...
                        >
                          {selectedPlan?.id === plan.id && <Check className="absolute right-4 top-4 h-5 w-5" />}
                          <div className="pr-7">
                            <h3 className="text-base font-bold leading-6">{plan.name}</h3>
                            <p className="mt-2 min-h-[42px] text-xs leading-5 opacity-80">{plan.summary}</p>
                          </div>
                          <div className="mt-4 text-2xl font-bold">
                            {formatRupees(planPrice(plan))}
                            <span className="text-xs font-semibold opacity-75"> once</span>
                          </div>
                          <div className="mt-3 space-y-1 text-xs opacity-80">
                            <div>{plan.duration}</div>
                            <div>{plan.cadence}</div>
                          </div>
                          <div className="mt-4 flex flex-wrap gap-1.5">
                            {(plan.bestFor || []).slice(0, 3).map((item) => (
                              <span key={item} className={`rounded-full px-2 py-1 text-[10px] font-semibold ${selectedPlan?.id === plan.id ? "bg-white/15 text-white" : "bg-violet-500/20 text-violet-300"}`}>
                                {item}
                              </span>
                            ))}
                          </div>
                  ...
                  <Card className="dashboard-card-motion rounded-[24px] border-white/10 bg-[#0b1020]">
                    <CardContent className="grid gap-4 p-5 md:grid-cols-3">
                      <ReviewCard text="Clear, kind, and practical support. I left with steps I could actually follow." name="Verified user" />
                      <ReviewCard text="The plan felt structured without being overwhelming. Scheduling was simple." name="Student client" />
                      <ReviewCard text="Good listener, quick response, and very respectful of privacy." name="Anonymous review" />
                    </CardContent>
                  </Card>
                </div>

                <aside className="lg:sticky lg:top-24 lg:self-start">
                  <Card className="dashboard-card-motion overflow-hidden rounded-[28px] border-white/10 bg-[#0b1020] shadow-[0_22px_60px_rgba(4,7,18,0.36)]">
                    <CardContent className="relative p-5">
                      ...
                      <div className="relative">
                        ...
                        <h2 className="mt-2 text-2xl font-bold leading-tight">{selectedPlan?.name}</h2>
                        <p className="mt-2 text-sm leading-6 text-slate-300/80">{selectedPlan?.summary || "Personalized support plan"}</p>

                        <div className="mt-5 rounded-[24px] border border-violet-300/25 bg-gradient-to-br from-violet-500/25 to-blue-500/10 p-4">
                          <div className="text-xs uppercase tracking-[0.16em] text-violet-100/80">Package total</div>
                          <div className="mt-2 text-4xl font-bold text-white">{formatRupees(planPrice(selectedPlan))}</div>
                        ...
                        <div className="mt-5 space-y-3 text-sm">
                          ...
                          <SummaryLine label="Mode" value={consultationModes.map(modeLabel).join(", ")} />
                        </div>
                        ...
                        <p className="mt-3 text-center text-xs leading-5 text-slate-300/65">
                          Booking unlocks the full support package. No per-session billing is shown.
                        </p>
                      </div>
                    </CardContent>
                  </Card>
...
function MiniPoint({ icon: Icon, title, text }) {
  return (
    <div className="rounded-2xl border border-white/8 bg-[#070b15] p-3">
      <div className="flex items-center gap-2 text-sm font-bold">
        <Icon className="h-4 w-4 text-violet-300" />
        {title}
      </div>
      <p className="mt-1 text-xs leading-5 text-slate-300/65">{text}</p>
    </div>
  );
}
...
function InfoTile({ icon: Icon, label, value }) {
  return (
    <div className="rounded-[18px] border border-white/8 bg-[#151b30]/75 p-4">
...
function ReviewCard({ text, name }) {
  return (
    <div className="rounded-[18px] border border-white/8 bg-[#070b15] p-4">
      ...
      <p className="mt-3 min-h-[60px] text-sm leading-6 text-slate-300/80">"{text}"</p>
      <div className="mt-3 text-xs font-semibold text-slate-400">{name}</div>
    </div>
  );
}
